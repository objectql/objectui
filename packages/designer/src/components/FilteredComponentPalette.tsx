...
interface FilteredComponentPaletteProps {
  className?: string;
  /**
   * List of allowed component types. If provided, only these components will be shown.
   */
  allowedComponents?: string[];
  /**
   * Categories to display. If provided, only these categories will be shown.
   */
  categories?: Record<string, string[]>;
  /**
   * Title to display at the top of the palette
   */
  title?: string;
}
...
export const FilteredComponentPalette: React.FC<FilteredComponentPaletteProps> = React.memo(({ 
  className, 
  allowedComponents,
  ...
  // Filter available components based on category and allowedComponents
  const getComponentsForCategory = React.useCallback((categoryComponents: string[]) => {
      return categoryComponents.filter(type => {
        // Check if component exists in registry
        if (!ComponentRegistry.getConfig(type)) return false;
        // Check if component is in allowedComponents list (if provided)
        if (allowedComponents && !allowedComponents.includes(type)) return false;
        return true;
      });
  }, [allowedComponents]);
        ...
        <ScrollArea className="flex-1 w-full">
            <div className="p-3 md:p-4 space-y-4 md:space-y-6 pb-20">
                {Object.entries(categories).map(([category, types]) => {
                    const availableTypes = filterBySearch(getComponentsForCategory(types));
                ...
                {/* Fallback for uncategorized components (only if no categories filter provided) */}
                {(() => {
                    const categorized = new Set(Object.values(categories).flat());
                    const allTypes = Object.keys(allConfigs);
                    const filteredTypes = allowedComponents ? allTypes.filter(t => allowedComponents.includes(t)) : allTypes;
                    const uncategorized = filterBySearch(filteredTypes.filter(t => !categorized.has(t)));
...
FilteredComponentPalette.displayName = 'FilteredComponentPalette';
