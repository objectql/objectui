...
- [x] `$expand` auto-injection: `buildExpandFields()` utility in `@object-ui/core` scans schema fields for `lookup`/`master_detail` types and returns field names for `$expand`. Integrated into ListView, ObjectGrid, and ObjectKanban data fetches so the backend (objectql) returns expanded objects instead of raw foreign-key IDs. Supports column-scoped expansion and graceful fallback when `$expand` is not supported. Cross-repo: objectql engine expand support required.
