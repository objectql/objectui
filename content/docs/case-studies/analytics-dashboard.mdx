...
- 📈 Custom chart builder with 15+ visualization types
...
├── src/
│   ├── schemas/
│   │   ├── overview.schema.ts        # Main analytics overview
│   │   ├── dashboard-builder.schema.ts # Custom dashboard builder
│   │   ├── chart-config.schema.ts    # Chart configuration
│   │   ├── reports.schema.ts         # Report management
│   │   ├── segments.schema.ts        # User segmentation
│   │   ├── goals.schema.ts           # Goal tracking
│   │   └── data-explorer.schema.ts   # Data exploration tool
│   ├── api/
│   │   ├── metrics/
│   │   ├── events/
│   │   ├── dashboards/
...
│   │   ├── aggregations.ts          # Data aggregation utils
│   │   ├── export.ts                # PDF/Excel export
│   │   └── websocket.ts             # Real-time updates
│   └── workers/
│       ├── report-generator.ts      # Background report generation
│       └── metric-aggregator.ts     # Metric calculation
├── prisma/
│   └── schema.prisma
└── package.json
```
...
- [**Video Tutorial**](/docs/guide/video-tutorials#analytics-dashboard)
