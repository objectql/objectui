...
<Callout type="info">
  ...
  objectui#8934 the panel applies it with the same pipeline `record:activity`
  uses. Its **defaults** come with it: `showCompleted` is `false`, so completed
  activities are hidden unless asked for, and an unauthored `limit` is `20`, so
  a longer feed pages with **Load more** rather than rendering whole.
</Callout>
