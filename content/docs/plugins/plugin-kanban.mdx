...
### Quick Add is a `kanban-ui` capability, not an object-driven one
...
directly, which is the `kanban-ui` block above.
...
a handler attribute is refused there. An authored `quickAdd: true` on those tags
...
<Callout type="info">
  `quickAdd` is being retired from `object-kanban` / `kanban`; it stays on
  `kanban-ui`, where the pair works. Inline record creation from a board is not a
  capability the object-driven boards have.
</Callout>
