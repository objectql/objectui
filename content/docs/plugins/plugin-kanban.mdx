...
const schema: ObjectKanbanSchema = {
  ...
  onCardMove: (cardId: string, fromCol: string, toCol: string, index: number) => {
...
export function useBoard(initialColumns: KanbanColumn[]) {
  ...
  const schema: ObjectKanbanSchema = {
    ...
    onCardMove: (cardId: string, fromColumnId: string, toColumnId: string, newIndex: number) => {
      // Update database
...
```plaintext
import type { ObjectKanbanSchema, KanbanCard, KanbanColumn } from '@object-ui/plugin-kanban'
